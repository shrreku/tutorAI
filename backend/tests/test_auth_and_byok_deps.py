...
def test_check_auth_rate_limit_raises_when_threshold_reached(monkeypatch):
    ...
    monkeypatch.setattr(
        settings, "AUTH_RATE_LIMIT_REQUESTS_PER_MINUTE", 1, raising=False
    )
...
def test_get_configured_admin_external_id_prefers_single_explicit_value(monkeypatch):
    ...
    monkeypatch.setattr(
        settings, "ADMIN_EXTERNAL_IDS", "legacy@example.com", raising=False
    )
...
def test_get_configured_admin_external_id_rejects_multiple_legacy_values(monkeypatch):
    ...
    monkeypatch.setattr(
        settings, "ADMIN_EXTERNAL_IDS", "a@example.com,b@example.com", raising=False
    )
