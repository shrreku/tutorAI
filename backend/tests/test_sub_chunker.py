...
def test_sub_chunker_stays_below_500_tokens():
    chunk = ChunkData(
        chunk_index=0,
        text=' '.join([f'Sentence {i}. This is a longer sentence for splitting.' for i in range(120)]),
    ...
    result = SubChunker(target_tokens=448, min_tokens=128, overlap_tokens=64).sub_chunk([chunk])
