...
class SafetyCritic:
    ...
    def __init__(self, llm_provider: BaseLLMProvider, llm_timeout_seconds: float = 25.0):
    ...
    async def evaluate(
        ...
        if unsupported_cited_ids:
            return SafetyCriticOutput(
                ...
                question_category=self._categorize_question(response_text) if contains_question else None,
        ...
        try:
            ...
            return SafetyCriticOutput(
                ...
                grounding_confidence=output.get("grounding_confidence", grounding_score),
                supporting_chunk_ids=output.get("supporting_chunk_ids", []),
                safety_decision=output.get(
                    "safety_decision",
                    "refuse_and_redirect" if output.get("should_block", False) else "allow",
                ),
            )
    ...
    def _quick_grounding_check(self, response: str, chunks: list[str]) -> float:
        ...
        chunk_words = set(
            word.lower()
            for word in re.findall(r'\b\w{4,}\b', chunk_text)
        )
