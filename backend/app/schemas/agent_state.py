...
class PolicyState(BaseModel):
    ...
    session_mode: str = Field(
        default="learn",
        description="Requested session mode: learn, doubt, practice, revision",
    )
    ...
    objective_queue_summary: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Brief summary of all objectives [{id, title, primary_concepts}]",
    )
...
class TutorState(BaseModel):
    ...
    recommended_strategy: Optional[str] = Field(
        default=None,
        description="Pedagogical strategy: direct, socratic, scaffolded, assessment, review",
    )
...
class EvaluatorState(BaseModel):
    ...
    current_step: Optional[str] = Field(
        default=None, description="Canonical roadmap step type"
    )
    effective_step_type: Optional[str] = Field(
        default=None, description="Runtime-effective step type after ad-hoc overrides"
    )
