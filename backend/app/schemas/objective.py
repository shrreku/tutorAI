...
class CurriculumStep(BaseModel):
    ...
    target_concepts: List[str] = Field(
        ..., min_length=1, description="Concepts to focus on in this step"
    )
    can_skip: bool = False
    max_turns: int = Field(default=3, ge=1, le=4)
    goal: str = Field(
        default="Drive understanding for this step.",
        min_length=1,
        description="Short completion goal for this step",
    )
    expected_student_action: Optional[ExpectedStudentAction] = None
    step_evidence_chunk_ids_topk: Optional[List[str]] = Field(
        default=None, description="Top-k chunk IDs grounding this step"
    )
...
class Objective(BaseModel):
    ...
    objective_evidence_chunk_ids_topk: List[str] = Field(
        ..., min_length=1, description="Chunk IDs grounding this objective"
    )
    ...
    prereq_objective_ids: Optional[List[str]] = Field(
        default=None, description="Objectives that should precede this one"
    )
...
class ObjectiveProgress(BaseModel):
    ...
    mastery_snapshot: Optional[Dict[str, float]] = Field(
        default=None, description="Concept → mastery mapping"
    )
