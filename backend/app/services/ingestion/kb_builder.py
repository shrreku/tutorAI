...
class ResourceKBBuilder:
    ...
    async def build(
        ...
        admitted, concept_metadata = await self._admit_concepts(resource_id, enrichments)
        ...
        await self._build_concept_stats(resource_id, chunks, enrichments, admitted, concept_metadata)
    ...
    async def _clear_kb_artifacts(self, resource_id: uuid.UUID) -> None:
        ...
        await self.db.execute(
            delete(ResourceConceptStats).where(ResourceConceptStats.resource_id == resource_id)
        )
        await self.db.execute(
            delete(ResourceConceptEvidence).where(ResourceConceptEvidence.resource_id == resource_id)
        )
        await self.db.execute(
            delete(ResourcePrereqHint).where(ResourcePrereqHint.resource_id == resource_id)
        )
        await self.db.flush()
    ...
    async def _admit_concepts(
        ...
        for i, enrichment in enumerate(enrichments):
            ...
            for meta in enrichment.get("concept_metadata", []):
                if isinstance(meta, dict):
                    ...
                    if cid:
                        concept_data[cid]["concept_types"].append(meta.get("concept_type", "principle"))
                        concept_data[cid]["bloom_levels"].append(meta.get("bloom_level", "understand"))
        ...
        for concept, data in concept_data.items():
            ...
            if data["importances"]:
                avg_importance = sum(
                    importance_weights.get(imp, 0.5) 
            ...
            if data["teach"] >= min_teach or base_score >= min_score:
                ...
                concept_metadata[concept] = {
                    "concept_type": self._most_common(data["concept_types"]) or "principle",
                    "bloom_level": self._highest_bloom(data["bloom_levels"]) or "understand",
        ...
        for i, (chunk, enrichment) in enumerate(zip(chunks, enrichments)):
            ...
            for concept in enrichment.get("concepts_taught", []):
                if concept in admitted:
                    evidence_rows.append(ResourceConceptEvidence(
    ...
    async def _build_concept_stats(
        ...
        for concept, data in stats.items():
            ...
            avg_quality = statistics.mean(data["quality_scores"]) if data["quality_scores"] else None
            ...
            primary_type = meta.get("concept_type") or (max(type_dist, key=type_dist.get) if type_dist else None)
            primary_bloom = meta.get("bloom_level") or (max(bloom_dist, key=bloom_dist.get) if bloom_dist else None)
            
            stats_rows.append(ResourceConceptStats(
                resource_id=resource_id,
                concept_id=concept,
    ...
    async def _build_prereq_hints(
        ...
        prereq_counts: dict[tuple, dict] = defaultdict(lambda: {"count": 0, "sources": []})
        ...
        for relation in ontology_relations or []:
            ...
            if not source or not target or source not in admitted or target not in admitted:
                continue
        ...
        for (source, target), data in prereq_counts.items():
            hint_rows.append(ResourcePrereqHint(
                resource_id=resource_id,
