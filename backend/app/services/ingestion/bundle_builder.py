...
class BundleBuilder:
    ...
    async def build_concept_bundles(
        ...
        for concept in concepts:
            ...
            for n in neighbors[:top_k_support]:
                other = n.target_concept_id if n.source_concept_id == concept else n.source_concept_id
                if other not in support:
                    support.append(other)
            
            # Get prereq hints for this concept
            concept_prereqs = [p.target_concept_id for p in prereqs if p.source_concept_id == concept]
            ...
            prototypes = {
                "teaches": [str(e.chunk_id) for e in concept_evidence if e.role == "teaches"][:3],
                "exemplifies": [str(e.chunk_id) for e in concept_evidence if e.role == "exemplifies"][:2],
                "mentions": [str(e.chunk_id) for e in concept_evidence if e.role == "mentions"][:2],
            }
        ...
        logger.info(f"Created {len(bundles)} concept bundles for resource {resource_id}")
    ...
    async def build_topic_bundles(
        ...
        for i, (seed_concept, concepts_in_topic) in enumerate(topic_concepts.items()):
            ...
            topic_evidence = [e for e in evidence if e.concept_id in concepts_in_topic and e.role == "teaches"]
            topic_evidence.sort(key=lambda x: -(x.quality_score or 0.5))
            representative_chunks = [str(e.chunk_id) for e in topic_evidence[:5]]
            
            topic_bundles.append(ResourceTopicBundle(
                resource_id=resource_id,
                topic_id=tid,
        ...
        logger.info(f"Created {len(topic_bundles)} topic bundles for resource {resource_id}")
    ...
    async def _get_prereq_hints(self, resource_id: uuid.UUID) -> list[ResourcePrereqHint]:
