...
class ResourceOntology:
    ...
    def get_enrichment_context(self, max_tokens: int = 800) -> str:
        ...
        if self.learning_objectives:
            obj_strs = [
                o.get("objective", "")[:80] for o in self.learning_objectives[:3]
            ]
            if obj_strs:
...
class OntologyExtractor:
    ...
    async def extract(
        ...
        for i, batch in enumerate(batches):
            ...
            try:
                ...
                logger.info(
                    f"[ONTOLOGY] Batch {i + 1}: {topics_found} topics, {concepts_found} concepts"
                )
    ...
    def _build_token_batches(
        ...
        for section in sections:
            ...
            if (
                current_tokens + section_tokens > self.max_tokens_per_call
                and current_batch
            ):
        ...
        if accumulated_objectives or accumulated_topics:
            ...
            if accumulated_topics:
                ...
                already_extracted.append(
                    f"TOPICS ALREADY IDENTIFIED: {', '.join(topic_names)}"
                )
            if accumulated_objectives:
                obj_list = [
                    o.get("objective", "")[:100] for o in accumulated_objectives[:10]
                ]
                already_extracted.append(
    ...
    async def _merge_results(
        ...
        for result in window_results:
            ...
            for topic in result.main_topics:
                topic_dict = (
                    topic.model_dump() if hasattr(topic, "model_dump") else dict(topic)
                )
            ...
            for relation in result.semantic_relations:
                ...
                section_heading = (
                    (relation_dict.get("section_heading") or "").strip().lower()
                )
