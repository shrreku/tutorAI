...
class ChunkEnricher:
    ...
    async def enrich_batch(
        ...
        if ontology_context:
            logger.info(
                f"[ENRICHMENT] Using ontology context ({len(ontology_context)} chars)"
            )
    ...
    async def _enrich_single(
        ...
        try:
                    ...
                    concept_metadata.append(
                        {
                            ...
                            "raw_name": raw_name,
                            "concept_type": self._validate_concept_type(
                                concept_obj.get("concept_type", "principle")
                            ),
                            "bloom_level": self._validate_bloom_level(
                                concept_obj.get("bloom_level", "understand")
                            ),
            ...
            return ChunkEnrichment(
                ...
                pedagogy_role=self._validate_pedagogy_role(
                    enrichment_data.get("pedagogy_role", "explanation")
                ),
                difficulty=self._validate_difficulty(
                    enrichment_data.get("difficulty", "intermediate")
                ),
                quality_score=self._validate_quality_score(
                    enrichment_data.get("quality_score", 0.5)
                ),
