...
import { useSession, useTurns, useEndSession, useSendMessage } from '../api/hooks';
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {session.status === 'completed' && (
          <div className="border-t border-border bg-card/50 px-6 py-4">
            <div className="max-w-3xl mx-auto flex items-center justify-center gap-3 text-sm text-muted-foreground">
          ...
          {masteryEntries.length > 0 && (
            <div className="animate-fade-up" style={{ animationDelay: '0.05s' }}>
              ...
              <div className="rounded-xl border border-border bg-card p-4 space-y-3">
                {masteryEntries.slice(0, 6).map(([concept, value]) => (
                  <div key={concept} className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-card-foreground truncate capitalize max-w-[140px]">
                        {concept.replace(/_/g, ' ')}
                      </span>
            ...
            <div className="rounded-xl border border-border bg-card p-4 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-md border text-[11px] font-medium ${
                  ...
                  {session.status === 'active' && <div className="w-1.5 h-1.5 rounded-full bg-gold" />}
                  {session.status}
                </div>
              </div>
              <div className="flex justify-between items-center">
              ...
              {objectiveQueue.length > 0 && (
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-card-foreground font-mono">{currentObjIndex + 1} / {objectiveQueue.length}</span>
                </div>
              )}
            </div>
          </div>
